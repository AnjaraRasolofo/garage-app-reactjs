...
const RepairForm = () => {
    ...
    try {
      const payload = {
        ...
        lines: formData.lines.map((line) => ({
          ...
          employees: [
            {
              employee_id: line.employee_id || 1, 
  ...
  const generateInvoicePDF = () => {
    ...
    doc.text("FACTURE DE RÉPARATION", 14, 15);

    doc.text(`Client: ${formData.client}`, 14, 25);
    doc.text(`Véhicule: ${formData.vehicle?.number || ""}`, 14, 32);
    doc.text(`Date: ${formData.date}`, 14, 39);
    ...
    doc.save(`facture-reparation-${formData.vehicle?.number || "client"}.pdf`);
  };
  ...
  <div className="card-body d-flex justify-content-between align-items-center">
    ...
    <div className="d-flex gap-2">
      ...
      <button
        ...
        onClick={() => {
          if (window.confirm("Réinitialiser la réparation ?")) {
            setFormData({
              vehicle_id: '',
              vehicle: '',
              client: '',
